...
namespace MatrixCalculator
{
    public class Matrix
    {
        ...
        public static Matrix operator *(Matrix m1, Matrix m2)
        {
            ...
            {
                newMatrix[i] = new MaskedTextBox[newWidth];
                for (int j = 0; j < newWidth; j++)
                {
                    newMatrix[i][j] = new MaskedTextBox();
                    newMatrix[i][j].Text = "0";
                    for (int k = 0; k < m1.Width ; k++) { 
                        newMatrix[i][j].Text = (Convert.ToInt16(newMatrix[i][j].Text)
                            + (Convert.ToInt16(m1[i][k].Text)
                            * Convert.ToInt16(m2[k][j].Text))).ToString();
                    }
                }
            }*/ 
